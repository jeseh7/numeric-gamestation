    ...
    <div className={`landingBody ${theme}`}>
      {currentUser && (
        <div className={`dropdownContainer ${theme === 'dark' ? 'darkTheme' : ''}`}>
          <Dropdown>
            <Dropdown.Toggle variant={theme === 'dark' ? 'light' : 'dark'} id="dropdown-basic" className="dropdownToggle rounded" style={{ padding: '0.05rem 0.2rem', fontSize: '0.975rem' }}>
      ...
      <video className="App-video" id='numeric' muted playsInline autoPlay loop style={{ filter: theme === 'dark' ? 'invert(84.7%)' : 'none' }}>
